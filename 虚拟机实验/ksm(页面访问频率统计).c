...
#define MY_FILE "/home/test/stat.txt"
...
static int ksm_scan_thread(void *nothing)
{
	...
	while (true) {
		...
		if(log_file != NULL && !IS_ERR(log_file) && target_pid){
			int ret;
			ret = findMysql();
			...
			for(;scan_pfn < max_pfn; scan_pfn++){
				if(pfn_valid(scan_pfn)){
					...
					if(page && is_file_page(page)){
						pc = lookup_page_cgroup(page);
						sprintf(str_buf, "11111111:%ul:%d\n", page_to_pfn(page), page_referenced(page, 0, pc, &vm_flags));
